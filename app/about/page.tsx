...
const sections: { eyebrow: string; body: string[] }[] = [
  ...
  {
    eyebrow: 'Passion projects matter',
    body: [
  ...
  return (
    <>
      ...
      <main className="relative z-10 mx-auto max-w-6xl px-6 pt-24 pb-24 sm:pt-32">
        ...
        <div className="mt-20 grid gap-x-16 gap-y-16 lg:grid-cols-[1fr_15rem]">
          {/* Reading column */}
          <div>
            ...
            <div className="mt-4 flex flex-col divide-y divide-[var(--border)] border-y border-[var(--border)]">
              {links.map((l) => (
                ...
                >
                  ...
                  <ArrowUpRight className="h-4 w-4 shrink-0 -translate-x-1 text-[var(--faint)] opacity-0 transition-all group-hover:translate-x-0 group-hover:text-[var(--accent)] group-hover:opacity-100" />
                </a>
              ))}
            </div>
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-[var(--accent)] px-5 py-3 text-sm font-semibold text-white transition-opacity hover:opacity-90"
            >
